...
export default function ChapterBreakdownSection() {
  return (
    <section className="py-20 bg-gradient-to-br from-sage/5 to-clay/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {chapters.map((chapter, index) => (
            ...
            >
              <div className="text-3xl font-bold sacred-text text-sage mb-3">{chapter.number}</div>
              <h3 className="text-lg font-semibold sacred-text text-forest mb-3">{chapter.title}</h3>
              <p className="text-earth body-text text-sm">{chapter.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
